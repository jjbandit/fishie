Meteor.methods ({
	createLesson: function(lessonID, level, weekdays, startTime, length, swimmers, privateLesson) {
		...
		var splitCandidates = Fishie.getSplitCandidatesForLesson(lessonObj, excludeIDs, lessonObj.levels, 0);
		var trimmedSplit = [];
		console.log('splitCandidates');
		console.log(splitCandidates);
		var path = [lessonObj];
		var epUp = Fishie.optimizeSplits(splitCandidates, lessonObj.levels, 1, path);
		var epDn = Fishie.optimizeSplits(splitCandidates, lessonObj.levels, -1, path);
		var shortPath;
		...
		if (shortPath) {
			Fishie.condenseLessons(shortPath, lessonID_str);
		}
	},
});
