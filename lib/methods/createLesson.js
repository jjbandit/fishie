Meteor.methods ({
	createLesson: function(lessonID, level, weekdays, startTime, length, swimmers, privateLesson) {
		...
		var lessonTimes = Fishie.getTimeBlocks(startTime, endTime, weekdays);
		var maxSwimmers = Fishie.getMaxSwimmers(level);
		lessonObj = Fishie.parseLessonObject(lessonID_str, level, privateLesson, swimmers, maxSwimmers, lessonTimes, length);
		...
		if (!epUp && !epDn) {
			...
			Fishie.scheduleLesson(lessonObj);
			return;
		...
		if (shortPath) {
			...
			// FIXME PRETTY SURE THIS IS WHY THINGS ARE RENDERING MORE THAN ONCE, COULD HAVE BROKEN SHIT THOUGH
			// if (!condensed) {
			// 		// If we didn't find a path to split with insert a new lesson
			// 	Lessons.insert (
			// 		lessonObj
