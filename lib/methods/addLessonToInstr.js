Meteor.methods({
	addLessonToInstr: function(instrIdOrObj, lessonObj, setID) {
		...
		// If we inserted a new instructor push it into the lessonSet collection
		if (result.insertedId) {
			LessonSets.update(setID, {
				$push: {
					instructorList: result.insertedId
				}
			});
		}
	}
