...
Fishie.optimizeSplits = function(splitCandidates, direction, path){
	if (splitCandidates.length > 0) {
		for (var i = 0; i < splitCandidates.length; i++) {
			if (splitCandidates[i].levels.length > 1) {
				...
				var recurseSplits = Fishie.getSplitCandidatesForLesson(splitCandidates[i], excludeIDs, direction);
				if (recurseSplits.length > 0) {
					return f = Fishie.optimizeSplits(recurseSplits, direction, path);
				}
			} else {
				console.log('foundAnEndpoint!~~~~~~~~~~~~~~~~~~~~~~~');
				console.log(splitCandidates[i]);
				return _.union(path, splitCandidates[i]);
			}
		}
	}
}
