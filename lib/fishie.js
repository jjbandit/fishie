...
Fishie.getLeadingBreaks = function(lessonObj) {
	...
	var timeIndex = instrLessonTimes.indexOf(lessonStartTime);
	...
	if (timeIndex > 0) {
		...
		prevLessonEndBlock = instrLessonTimes[timeIndex - 1];
...
Fishie.getTrailingBreaks = function(lessonObj, alwaysReturn) {
	...
	// Otherwise, if we want to return trailing breaks for any old lesson, do so
		...
		console.log('et');
		console.log(lessonObj.endTime());
		return Fishie.getTimeBlocks(lessonObj.endTime(), new Date(instrLessonTimes[timeIndex + 1]));
	}
}
...
Fishie.getTimeBlocks = function(firstTime, lastTime, weekdays) {
	...
	for (var w = 0; w < weekdays.length; w++) {
		for (var t = 0; t < timeBlocks; t++) {
			...
			var blockTime = new Date(1989, 0, 0, 0, 0, 0, 0);
			var currentDay;
			var distance;
