...
Fishie.getSplitCandidates = function(lessonTime, level, swimmers, maxSwimmers) {
	var levelHigh = level + 1;
	var levelLow = level - 1;
	var splitCandidates = Lessons.find({lessonTime: lessonTime, levels: {$in: [levelHigh, levelLow]}, swimmers: {$lte: maxSwimmers - swimmers} }).fetch();
	return splitCandidates;
}
Fishie.getSplitCandidatesForLesson = function (lessonObj, excludeIDs) {
	var levelHigh = Math.max.apply(Math, lessonObj.levels) + 1;
	var levelLow = Math.min.apply(Math, lessonObj.levels) - 1;
	console.log('gettingSplitCandidate for');
	console.log(lessonObj);
	var splitCandidates = Lessons.find({_id: {$nin: excludeIDs}, levels: {$in: [levelHigh, levelLow]}, lessonTime: lessonObj.lessonTime }).fetch();
	console.log('gotem');
	console.log(splitCandidates);
	return splitCandidates;
}
Fishie.optimizeSplits = function(splitCandidates, path){
	if (splitCandidates.length > 0) {
		...
		for (var i = 0; i < splitCandidates.length; i++) {
			if (splitCandidates[i].levels.length > 1) {
				...
				var newSplits = Fishie.getSplitCandidatesForLesson(splitCandidates[i], excludeIDs);
				recurseSplits = _.union(newSplits, recurseSplits);
				// path = _.union(splitCandidates[i], path);
			} else {
				console.log('foundAnEndpoint!~~~~~~~~~~~~~~~~~~~~~~~');
				console.log(splitCandidates[i]);
				return _.union(splitCandidates[i], path);
			}
		}
		// this has to be outside the for loop so it actually loops
		return Fishie.optimizeSplits(recurseSplits, path);
	}
}
