...
Fishie.getLeadingBreaks = function(lessonObj) {
	...
	if (prevLesson === undefined) {
		var overallFirstLesson = Lessons.findOne({owner: Meteor.userId()}, {sort: {lessonTimes: 1}});
...
Fishie.getTrailingBreaks = function(lessonObj, alwaysReturn) {
	...
	var nextLesson = Lessons.findOne({
		instructor: lessonObj.instructor,
		owner: Meteor.userId(),
		'lessonTimes.0': {$gte: lessonObj.endTime()}
	}, {
		sort: {lessonTimes: 1}
	});
	...
	// Otherwise, if we want to return trailing breaks for any old lesson, do so
		...
		return Fishie.getTimeBlocks(lessonObj.endTime(), nextLesson.startTime());
	}
}
