...
Fishie.addGhostLessons = function(instrAry, lessonTimes) {
	// TODO CREATE NEW LESSON IN LESSONS COLLECTION WITH SPECIAL FLAG
	// THEN RENDER THIS AS A GREEN BLOCK :)
	var _idString = new Meteor.Collection.ObjectID()._str;
	var lessonObj = Fishie.parseLessonObject(undefined, 'ghost', '', [], '', lessonTimes, 0);
	console.log(lessonObj);
	Fishie.addLessonToInstr(instrAry[0]._id, lessonObj);
}
...
Fishie.newInstructor = function(lessonObj) {
	...
	// optionally supply a lesson object to initialize the lessonTimes property
