...
Fishie.scheduleLesson = function(lessonID) {
	...
	if (availableInstructors.length == 0) {
		...
		Fishie.addLessonToInstr(instrID, lessonTime);
	}
}
//
...
// optionally supply a lesson.lessonTime to initialize the lessonTime property
Fishie.newInstructor = function(lessonTime) {
	if (!lessonTime) {
		var lessonTime = [];
	}
	Instructors.insert({
