Fishie = {}
Fishie.scheduleLesson = function(lessonObj, setID) {
...
Fishie.getTrailingBreaks = function(lessonObj, alwaysReturn) {
	...
	var wkDay = instrLessonTimes[0].getDay();
	...
	var ln = instrLessonTimes.length ;
	for (var i = 0; i < ln ; i++) {
		if (instrLessonTimes[i].getDay() != wkDay){
			instrLessonTimes[i] = null;
		}
	}
	instrLessonTimes = _.without(instrLessonTimes, null);
...
Fishie.addGhostLessons = function(instrArray, lessonTimes, length) {
	...
	for (var i = 0; i < instrArrayLength; i++) {
		...
		var lessonObj = Fishie.parseLessonObject(undefined, 'ghost', '', [], '', [0], lessonTimes, length);
...
Fishie.addLessonToInstr = function(instrID, lessonObj, setID) {
...
Fishie.parseLessonObject = function(_idString, level, privateLesson, swimmers, maxSwimmers, weekdays, lessonTimes, length) {
