...
Fishie.optimizeSplits = function(splitCandidates){
	// FIXME We should use a local variable thats passed between methods so it doesn't pollute global space
	if (typeof excludeIDs == "undefined") excludeIDs = [];
	console.log('function optimizeSplits');
	console.log(splitCandidates);
	if (splitCandidates.length > 0) {
		...
		for (var i = 0; i < splitCandidates.length; i++) {
			if (splitCandidates[i].levels.length > 1) {
				...
				secondarySplits = _.union(secondarySplits, newSplit);
			}
		}
		Fishie.optimizeSplits(secondarySplits);
	}
	// FIXME THIS HACK NEEDS TO BE REVISITED
	// reset the global exclude variable
	excludeIDs = [];
}
