...
Fishie.getSplitCandidates = function(lessonTime, level, swimmers) {
...
Fishie.getSplitCandidatesForLesson = function () {
	return 'yay';
}
	...
	if (splitCandidates.length > 0) {
		...
		for (var i = 0; i < splitCandidates.length; i++) {
			if (splitCandidates[i].levels.length > 1) {
				var lessonTime = splitCandidates[i].lessonTime;
				var level = splitCandidates[i].levels[0];
				var swimmers = splitCandidates[i].swimmers[0];
				var newSplit = Fishie.getSplitLessons(lessonTime, level, swimmers);
				secondarySplits.push(newSplit);
			}
		}
		console.log('secondarySplits');
		console.log(secondarySplits);
		// var nLevelSplits = [];
		// for (var i = 0; i < secondarySplits.length; i++) {
		// 	if (secondarySplits[i].levels.length > 1) {
		// 	}
		// 	nLevelSplits.push(secondarySplits[i]);
		// };
		// console.log('nLevelSplits');
		// console.log(nLevelSplits);
		// if (nLevelSplits.length > 0) {
		// 	Fishie.optimizeSplits(nLevelSplits);
	}
}
