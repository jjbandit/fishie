...
Fishie.condenseLessons = function (path, lessonID) {
	...
	// pathLevels = _.flatten(pathLevels);
	// pathSwimmers = _.flatten(pathSwimmers);
	zippedAry = _.zip(pathLevels, pathSwimmers);
	console.log(pathLevels);
	console.log(pathSwimmers);
	console.log(zippedAry.sort());
	for (var p = 0; p < path.length; p++) {
	...
	for (var i = 0; i < path.length; i++) {
		...
		// path[i].levels = lvls;
		// var swmrs = pathSwimmers.slice(i*2, i*2+2);
		// path[i].swimmers = swmrs;
		// Lessons.update(path[i]._id, {$set: {levels: lvls, swimmers: swmrs}});
	};
	console.log(path);
}
