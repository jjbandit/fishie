...
Fishie.getSplitCandidatesForLesson = function (lessonObj, excludeIDs, direction) {
	if (direction == 1) {
	...
	if (splitCandidates.length > 0) {
		...
		for (var i = 0; i < splitCandidates.length; i++) {
			if (splitCandidates[i].levels.length > 1) {
				...
				var newSplits = Fishie.getSplitCandidatesForLesson(splitCandidates[i], excludeIDs, direction);
				recurseSplits = _.union(newSplits, recurseSplits);
				f = Fishie.optimizeSplits(recurseSplits, path);
				if (f) {return;}
